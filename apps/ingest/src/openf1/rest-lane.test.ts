...
describe("RestLane discovery", () => {
  ...
  test("a malformed session (invalid date) fails before onSession is reached, and does not stop the rest or starve ensureLiveSession", async () => {
    ...
    // bad's own fields can't be computed (invalid date_start), so onSession
    // is never reached for it — but the good row after it still upserts.
    expect(onSession).toHaveBeenCalledTimes(1);
    expect(onSession).toHaveBeenCalledWith(SESSION, START, expect.any(Map));
    // ensureLiveSession() still ran on the good session despite the bad one
    // failing first.
