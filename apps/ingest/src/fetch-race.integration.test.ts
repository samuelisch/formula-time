...
test(
  ...
  async () => {
    ...
    // 2 drivers + 2 position + 1 lap + 1 stint.
    expect(first).toMatchObject({ inserted: 6, sessionsAttempted: 1, sessionsSkipped: 0, sessionsNotFound: 0 });
    ...
    // drivers first (2); order keys: stint = LAP_DATE_START (13:10:00, the
    // joined lap's raw date_start), position1 = 13:10:30, lap =
    // LAP_DATE_START + 60s duration = 13:11:00, position2 = 13:11:30.
    ...
    expect(countAfterSecond).toBe(6);
  },
  30_000,
);
