...
FROM node:24-slim
# GIT_SHA identifies the running build (issue #212): /health reports it so
# release.yml's smoke job can tell this build from the previous one.
# Railway passes the commit SHA as the build arg RAILWAY_GIT_COMMIT_SHA;
# GIT_SHA defaults to that but can be overridden explicitly (e.g. a local
