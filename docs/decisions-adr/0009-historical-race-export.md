...
1. **Format.** One file per session, `<session_key>.json.gz`: gzip of
   ...
   { "schema": 1,
     "session": { "session_key": 11361, "name": "Race", "country": "Italy", "circuit_key": 39,
                  "date_start": "…", "date_end": "…", "total_laps": 53, "status": "finished", "exported_at": "…" },
...
2. **Producer.** The api's exporter, on the same 5 s lifecycle tick that picks the session: for every `sessions` row with `status = 'finished'` and `exported_at IS NULL`, read the events once, write the file to `EXPORT_DIR` (default `./exports`) atomically (temp file, then rename), then set `exported_at`. Idempotent; a failure leaves `exported_at` null and the next tick retries. `EXPORT_DIR` joins the seam-4 config names.
3. **Disk is a cache, the database is the record.** Railway's disk is ephemeral. If a request finds `exported_at` set but the file missing, the api regenerates it once (same code path, logged) and serves it. That is one read per session per process lifetime, not per viewer.
4. **Serving.** Two routes under the client prefix:
   - `GET /api/races` → the finished sessions that have an export: `[{ session_key, name, country, date_start, date_end, total_laps, exported_at }]`, sorted by `date_start` descending.
   - `GET /api/races/:session_key` → the file as `application/json` with `content-encoding: gzip` (pre-compressed bytes, never re-encoded), `cache-control: public, max-age=31536000, immutable`, `etag: "<session_key>-<exported_at epoch ms>"`; 404 when the session is not finished or not yet exported.
...
- When object storage arrives, the exporter writes there instead of `EXPORT_DIR` and `GET /api/races/:session_key` becomes a redirect (ADR-0001 route handler: "finished → redirect to the export"). The file format and the browser do not change.
